/*
 * a simple char device driver: global memory without mutex
 */
...
// #define DEVICE_NUM 3 // number of devices, can be changed to more
...
static int global_mem_open(struct inode *inode, struct file *filp)
{
	...
	// 如果有多个设备需要获取到正确的结构体指针，然后赋值给 filp->private_data
	// struct global_mem_dev *dev =
	// 	container_of(inode->i_cdev, struct global_mem_dev, cdev);
	return 0;
}
...
static int __init global_mem_init(void)
{
	...
	// 如果有多个设备这里要分配连续的多个结构体指针
	// global_mem_devp = kmalloc_array(DEVICE_NUM, sizeof(struct global_mem_dev), GFP_KERNEL);
	// global_mem_devp = kzalloc(sizeof(struct global_mem_dev) * DEVICE_NUM, GFP_KERNEL);
	if (!global_mem_devp) {
	...
	// 	ret = global_mem_setup_cdev(&global_mem_devp[i], i);
	//	ret = global_mem_setup_cdev(global_mem_devp + i, i);
	// 	if (ret) {
	// 		printk(KERN_ERR "Failed to setup cdev for device %d\n", i);
	// 		break;
	// 	}
	// }
	if (ret) {
...
static void __exit global_mem_exit(void)
{
	if (global_mem_devp) {
		...
		// 如果有多个设备这里需要循环释放
		// for (int i = 0; i < DEVICE_NUM; i++) {
		// 	device_destroy(global_mem_devp[i].class, global_mem_devp[i].devno);
		// 	class_destroy(global_mem_devp[i].class);
