...
static long led_char_unlocked_ioctl (struct file *filp, unsigned int cmd, unsigned long args) {
    int tmp_time_period = 0;
    unsigned long flags;
    switch (cmd) {
        case IOCTL_CMD_LED_TIMER_OPEN:
            printk("CMD: IOCTL_CMD_LED_TIMER_OPEN\n");
            spin_lock_irqsave(&led_char->lock, flags);
            tmp_time_period = led_char->time_period; // 保存当前定时器周期
            spin_unlock_irqrestore(&led_char->lock, flags);
            mod_timer(&led_char->timer, jiffies + msecs_to_jiffies(tmp_time_period));
            break;
        case IOCTL_CMD_LED_TIMER_CLOSE:
            printk("CMD: IOCTL_CMD_LED_TIMER_CLOSE\n");
            del_timer(&led_char->timer);
            break;
        case IOCTL_CMD_LED_TIMER_SET:
            printk("CMD: IOCTL_CMD_LED_TIMER_SET\n");
            spin_lock_irqsave(&led_char->lock, flags);
            led_char->time_period = (int)args;
            spin_unlock_irqrestore(&led_char->lock, flags);
            mod_timer(&led_char->timer, jiffies + msecs_to_jiffies(led_char->time_period));
            break;
        default:
            break;
    }
...
static void led_char_timer_callback (struct timer_list *timer) {
...
static struct file_operations led_char_fops = {
    ...
    .unlocked_ioctl = led_char_unlocked_ioctl,
};
...
static int led_char_cdev_init (struct led_char *led_char) {
    ...
    // init timer, note that we use timer_setup() instead of init_timer()
    // not set timer period here, so it will not start until we call mod_timer()
    timer_setup(&led_char->timer, led_char_timer_callback, 0);
...
static void led_char_cdev_exit (struct led_char *led_char) {
    ...
    class_destroy(led_char->cls);
    // destory np and timer
    of_node_put(led_char->np);
    del_timer(&led_char->timer);
